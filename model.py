...
# cate, keyword, keyword2, tag1, tag2, tag3, ks1,ks2
# attention的权重都一样


class Model(object):

    def __init__(self, cate_count, keyword_count, keyword2_count, tag1_count, tag2_count, tag3_count, ks1_count,
                 ks2_count, user_features_num, item_features_num, user_features_dim, item_features_dim,
                 embedding_size=8, hidden_units=64 * 8, deep_layer=[80, 40, 1], lamda = 0.1):
        # cate_count：种类个数
        # keyword_count：关键词个数
        # keyword_count：关键词2个数
        # tag1_count：v4标签1个数
        # tag2_count：v4标签2个数
        # tag3_count：v4标签3个数
        # ks1_count：v4关键词1个数
        ...
        self.cate = tf.placeholder(tf.int32, [None, ], name='cate')
        self.keyword = tf.placeholder(tf.int32, [None, ], name='keyword')
        self.keyword2 = tf.placeholder(tf.int32, [None, ], name='keyword2')
        ...
        self.hist_cate = tf.placeholder(tf.int32, [None, None], name='hist_cate')  # [B, T]
        self.hist_keyword = tf.placeholder(tf.int32, [None, None], name='hist_keyword')  # [B, T]
        self.hist_keyword2 = tf.placeholder(tf.int32, [None, None], name='hist_keyword2')  # [B, T]
        ...
        tf.summary.histogram("cate_b", cate_b)
        tf.summary.histogram("cate_emb_w", cate_emb_w)
        tf.summary.histogram("keyword_emb_w", keyword_emb_w)
        tf.summary.histogram("keyword2_emb_w", keyword2_emb_w)
        ...
        i_emb = tf.concat(values=[
            ...
            tf.nn.embedding_lookup(cate_emb_w, self.cate),  # [B ，hidden_units // 2] = [B, H // 3]
            tf.nn.embedding_lookup(keyword_emb_w, self.keyword),
            tf.nn.embedding_lookup(keyword2_emb_w, self.keyword2),
            tf.nn.embedding_lookup(tag1_emb_w, self.tag1),
            tf.nn.embedding_lookup(tag2_emb_w, self.tag2),
            tf.nn.embedding_lookup(tag3_emb_w, self.tag3),
            tf.nn.embedding_lookup(ks1_emb_w, self.ks1),
            tf.nn.embedding_lookup(ks2_emb_w, self.ks2)
        ], axis=1)  # [B ，H]
        i_b = tf.gather(cate_b, self.cate)

        h_emb = tf.concat([  # 在shape【0，1，2】某一个维度上连接
            # tf.nn.embedding_lookup(item_emb_w, self.hist_item),  # [B, T, hidden_units // 3]
            tf.nn.embedding_lookup(cate_emb_w, self.hist_cate),  # [B, T, hidden_units // 3]
            tf.nn.embedding_lookup(keyword_emb_w, self.hist_keyword),  # [B, T, hidden_units // 3]
            tf.nn.embedding_lookup(keyword2_emb_w, self.hist_keyword2),  # [B, T, hidden_units // 3]
            tf.nn.embedding_lookup(tag1_emb_w, self.hist_tag1),  # [B, T, hidden_units // 3]
            tf.nn.embedding_lookup(tag2_emb_w, self.hist_tag2),  # [B, T, hidden_units // 3]
            tf.nn.embedding_lookup(tag3_emb_w, self.hist_tag3),  # [B, T, hidden_units // 3]
            tf.nn.embedding_lookup(ks1_emb_w, self.hist_ks1),  # [B, T, hidden_units // 3]
            tf.nn.embedding_lookup(ks2_emb_w, self.hist_ks2),  # [B, T, hidden_units // 3]
        ], axis=2)  # [B, T, H]
        # -- 嵌入层 end ---
        ...
        # -- fcn begin -------
        # -- 训练集全连接层 开始 -------
        din_i = tf.concat([u_emb, i_emb, user_feature, item_feature], axis=-1)
        din_i = tf.layers.batch_normalization(inputs=din_i, name='b0')

        d_layer_1_i = tf.layers.dense(din_i, deep_layer[0], activation=tf.nn.relu, name='f1')  # 全连接层  [B, 80]
        ...
        d_layer_2_i = tf.layers.dense(d_layer_1_i, deep_layer[1], activation=tf.nn.relu, name='f2')
    ...
    def train(self, sess, merge, (y, user_feature, item_feature, cate, keyword, keyword2, tag1, tag2, tag3, ks1, ks2,
        ...
        summary, loss, _ = sess.run([merge, self.loss, self.train_op], feed_dict={
            ...
            self.hist_cate: hist_cate,
            self.hist_keyword: hist_keyword,
            self.hist_keyword2: hist_keyword2,
    ...
    def eval_train(self, sess, (y, user_feature, item_feature, cate, keyword, keyword2, tag1, tag2, tag3, ks1, ks2,
                   hist_cate, hist_keyword, hist_keyword2, hist_tag1, hist_tag2, hist_tag3, hist_ks1, hist_ks2, sl,
                   keep_prob)):
    ...
    def eval(self, sess, (y, user_feature, item_feature, cate, keyword, keyword2, tag1, tag2, tag3, ks1, ks2, hist_cate,
             hist_keyword, hist_keyword2, hist_tag1, hist_tag2, hist_tag3, hist_ks1, hist_ks2, sl), keep_prob):
    ...
    def test(self, sess, merge, (y, user_feature, item_feature, cate, keyword, keyword2, tag1, tag2, tag3, ks1, ks2,
             hist_cate, hist_keyword, hist_keyword2, hist_tag1, hist_tag2, hist_tag3, hist_ks1, hist_ks2, sl,
             keep_prob)):
...
def attention(queries, keys, keys_length):
    ...
    d_layer_1_all = tf.layers.dense(din_all, 80, activation=tf.nn.relu, name='f1_att')  # [B, T, 80]
    # d_layer_1_all = dice(d_layer_1_all, name='dice_1_att')
    d_layer_2_all = tf.layers.dense(d_layer_1_all, 40, activation=tf.nn.relu, name='f2_att')  # [B, T, 40]
    ...
    key_masks = tf.expand_dims(key_masks, 1)  # [B, 1, T]`1
