...
class Model(object):
    ...
    def __init__(self, cate_count, keyword_count, keyword2_count, tag1_count, tag2_count, tag3_count, ks1_count,
        ...
        cate_emb = tf.nn.embedding_lookup(cate_emb_w, self.cate)  # [B ，hidden_units // 2] = [B, H // 3]
        keyword_emb = tf.nn.embedding_lookup(keyword_emb_w, self.keyword)
        keyword2_emb = tf.nn.embedding_lookup(keyword2_emb_w, self.keyword2)
        ...
        hist_cate_emb = tf.nn.embedding_lookup(cate_emb_w, self.hist_cate)  # [B, T, hidden_units // 3]
        hist_keyword_emb = tf.nn.embedding_lookup(keyword_emb_w, self.hist_keyword)  # [B, T, hidden_units // 3]
        hist_keyword2_emb = tf.nn.embedding_lookup(keyword2_emb_w, self.hist_keyword2) # [B, T, hidden_units // 3]
        ...
        # -- attention start ---
        hist_cate = attention(cate_emb, hist_cate_emb, self.sl)  # [B, 1, H]
        hist_cate = tf.layers.batch_normalization(inputs=hist_cate)
        hist_cate = tf.reshape(hist_cate, [-1, hidden_units])  # [B, H]
        hist_cate = tf.layers.dense(hist_cate, hidden_units)  # [B, H]
        ...
        hist_keyword2 = attention(keyword2_emb, hist_keyword2_emb, self.sl)  # [B, 1, H]
        hist_keyword2 = tf.layers.batch_normalization(inputs=hist_keyword2)
        hist_keyword2 = tf.reshape(hist_keyword2, [-1, hidden_units])  # [B, H]
        hist_keyword2 = tf.layers.dense(hist_keyword2, hidden_units)  # [B, H]
        ...
        # -- 训练集全连接层 开始 -------
        din_i = tf.concat(
            [hist_cate, hist_keyword, hist_keyword2, hist_tag1, hist_tag2, hist_tag3, hist_ks1, hist_ks2,
             cate_emb, keyword_emb, keyword2_emb, tag1_emb, tag2_emb, tag3_emb, ks1_emb, ks2_emb,
