...
# 设置提示信息等级，1显示所有，2显示WARN和ERROR，3显示ERROR
...
# 保留有3条历史行为的样本
data = data[data.userKs2History.str.count(",")>=2]
print('valid_total_count',data.shape[0])
del data['batchNo']
# del data['index']
# print('sample_total_count', data.shape[0])

train_set = data[data['userId'] % 2 == 0]
test_set = data[data['userId'] % 2 == 1]
del train_set['userId']
del test_set['userId']
del data
# train_set = train_set.sample(frac=0.7)
...
train_set['userCateHistory'] = train_set['userCateHistory'].apply(lambda x: split_to_int(x))
train_set['userKeywordHistory'] = train_set['userKeywordHistory'].apply(lambda x: split_to_int(x))
train_set['userKeyword2History'] = train_set['userKeyword2History'].apply(lambda x: split_to_int(x))
...
test_set['userCateHistory'] = test_set['userCateHistory'].apply(lambda x: split_to_int(x))
test_set['userKeywordHistory'] = test_set['userKeywordHistory'].apply(lambda x: split_to_int(x))
test_set['userKeyword2History'] = test_set['userKeyword2History'].apply(lambda x: split_to_int(x))
...
def _eval(sess, merge, model, total_step, test_writer):
    ...
    for i, (y, user_feature, item_feature, cate, keyword, keyword2, tag1, tag2, tag3, ks1, ks2, hist_cate, hist_keyword,
            hist_keyword2, hist_tag1, hist_tag2, hist_tag3, hist_ks1, hist_ks2, sl) in DataInput(
            slice_test_set, eval_batch_size):
        summary, score, loss = model.test(sess, merge,
                                          (y, user_feature, item_feature, cate, keyword, keyword2, tag1, tag2, tag3,
                                           ks1, ks2, hist_cate, hist_keyword, hist_keyword2, hist_tag1, hist_tag2,
...
def _eval_train(sess, model):
    ...
    for _, (y, user_feature, item_feature, cate, keyword, keyword2, tag1, tag2, tag3, ks1, ks2, hist_cate, hist_keyword,
            hist_keyword2, hist_tag1, hist_tag2, hist_tag3, hist_ks1, hist_ks2, sl) in DataInput(
            slice_train_set, eval_batch_size):
        score, loss = model.eval_train(sess, (y, user_feature, item_feature, cate, keyword, keyword2, tag1, tag2, tag3,
                                              ks1, ks2, hist_cate, hist_keyword, hist_keyword2, hist_tag1, hist_tag2,
                                              hist_tag3, hist_ks1, hist_ks2, sl, 1.0))
        loss_sum = loss_sum + loss
        score_arr += list(score)
...
def run():
    with tf.Session() as sess:
        ...
        inputs = {
            ...
            "hist_cate": tf.saved_model.utils.build_tensor_info(model.hist_cate),
            "hist_keyword": tf.saved_model.utils.build_tensor_info(model.hist_keyword),
            "hist_keyword2": tf.saved_model.utils.build_tensor_info(model.hist_keyword2),
            ...
            "cate": tf.saved_model.utils.build_tensor_info(model.cate),
            "keyword": tf.saved_model.utils.build_tensor_info(model.keyword),
            "keyword2": tf.saved_model.utils.build_tensor_info(model.keyword2),
        ...
        for epoch in range(10):
            ...
            for i, (y, user_feature, item_feature, cate, keyword, keyword2, tag1, tag2, tag3, ks1, ks2, hist_cate,
                ...
                train_summary, loss = model.train(sess, merge,
                                                  (y, user_feature, item_feature, cate, keyword, keyword2, tag1, tag2,
                ...
                if i % 100 == 0:
                    ...
                    print('epoch %d step %d t_step %d,train:b_loss:%.4f loss:%.4f auc:%.4f recall:%.4f pre:%.4f acc:%.4f' %
                          (epoch, i, step, loss_sum / 100, train_loss, train_auc, train_recall, train_precision,
                           train_accuracy))
                    print('epoch %d step %d t_step %d,test:               loss:%.4f auc:%.4f recall:%.4f pre:%.4f acc:%.4f' %
                          (epoch, i, step, test_loss, test_auc, test_recall, test_precision, test_accuracy))
                    sys.stdout.flush()
                    loss_sum = 0.0
                    ...
                    output_graph_def = tf.graph_util.convert_variables_to_constants(sess, sess.graph_def,
                                                                                    output_node_names=['sig_logits'])
                    with tf.gfile.FastGFile('save_path/din.pb' + '.' + str(int(step)), mode='wb') as f:
                        f.write(output_graph_def.SerializeToString())
                else:
